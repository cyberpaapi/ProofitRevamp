...
type SectionId = "overview" | "content" | "images" | "posts" | "careers" | "testimonials" | "cases" | "enquiries" | "proofy" | "appointments";
type Field = {
  key: string;
  label: string;
...
export default function AdminDashboard({
  ...
  const upcoming = store.appointments.filter((item) => item.status !== "cancelled" && item.date >= localDate(new Date())).length;
...
function EnquiriesPanel({ enquiries, setEnquiries }: { enquiries: StoredEnquiryRecord[]; setEnquiries: (items: StoredEnquiryRecord[]) => void }) {
  const [search, setSearch] = useState(""); const [status, setStatus] = useState("all"); const [selected, setSelected] = useState(enquiries[0]?.id || ""); const [saving, setSaving] = useState(false);
  const filtered = enquiries.filter((item) => (status === "all" || item.status === status) && `${item.name} ${item.email} ${item.phone} ${item.service}`.toLowerCase().includes(search.toLowerCase()));
