...
export function TeamPanel({items,onChange}:{items:AdminTeamMember[];onChange:(items:AdminTeamMember[])=>void}) {
  return <Editor title="Founders & leadership" items={items} onChange={onChange} label={item=>item.name} create={()=>({id:crypto.randomUUID(),name:'New team member',role:'',bio:'',image:'',visible:false,order:items.length})} render={(item,patch)=><>
    {(['name','role'] as const).map(key=><Field key={key} label={key==='role'?'Designation':'Name'}><input className={input} value={item[key]} onChange={e=>patch({[key]:e.target.value})} /></Field>)}
    <Field label="Biography"><textarea rows={9} className={input} value={item.bio} onChange={e=>patch({bio:e.target.value})} /></Field>
    <AdminImageField key={item.id} label="Team photo" value={item.image} onChange={image=>patch({image})} />
  </>} />;
}
export function OfferingsPanel({items,onChange}:{items:AdminOffering[];onChange:(items:AdminOffering[])=>void}) {
  return <Editor<AdminOffering> title="Services & homepage cards" items={items} onChange={onChange} label={item=>item.title} create={()=>({id:crypto.randomUUID(),category:'home-inspection',slug:`service-${crypto.randomUUID().slice(0,8)}`,title:'New service',description:'',image:'',benefits:[],homepage:false,visible:false,order:items.length})} render={(item,patch)=><>
    ...
    <AdminImageField key={item.id} label="Service image" value={item.image} onChange={image=>patch({image})} />
