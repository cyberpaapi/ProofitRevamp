...
export function CareServices({ services }: { services: { title: string; body: string; slug?: string }[] }) {
  return <section className="site-section bg-cream"><div className="site-container ">
    <SectionHeading eyebrow="Our Services" title="Complete care. One platform." />
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{services.map((service, index) => <article id={service.slug} key={service.slug || service.title} className="scroll-mt-28 rounded-2xl border border-ink/15 bg-white p-7 shadow-[0_6px_20px_rgba(0,0,0,.06)] transition duration-300 hover:-translate-y-1 hover:border-brand hover:shadow-lg motion-reduce:transform-none">
...
export function CareWhyCarousel({ reasons }: { reasons: { title: string; desc: string }[] }) {
  ...
  return <section className="site-section"><div className="site-container ">
    <SectionHeading eyebrow="Why Proofit Care+" title="Why property owners stay with us." />
    <div className="grid" aria-live="polite">{pages.map((page, pageIndex) => <div key={pageIndex} className={`col-start-1 row-start-1 grid grid-cols-1 gap-5 sm:grid-cols-2 transition-opacity duration-300 ${pageIndex === index ? "opacity-100" : "invisible opacity-0"}`} aria-hidden={pageIndex !== index}>
